...
year = Time.now.year
year_calendar = ''
Prawn::Document.generate('calendarA4.pdf', page_size: 'A4', margin: 0) do
...
# First page
  ...
  text_box "#{year}\nWeeks", at: [12.5.cm, 3.cm + page_height / 2],
                             ...
                             align: :right
  stroke do
    horizontal_line 0, page_width, at: page_height / 2
  end
  stamp_at 'dots', [gutter_cm + hmargin_cm - dot_size_cm, vmargin_cm]
  text_box 'thu', at: [gutter_cm + hmargin_cm + 5.cm, page_height / 2 - vmargin_cm],
                  width: 1.cm, height: 0.45.cm,
                  align: :center, valign: :center
  text_box 'fri', at: [gutter_cm + hmargin_cm + 11.cm, page_height / 2 - vmargin_cm],
                  width: 1.cm, height: 0.45.cm,
                  align: :center, valign: :center
  text_box 'sat', at: [gutter_cm + hmargin_cm + 17.cm, page_height / 2 - vmargin_cm],
                  width: 1.cm, height: 0.45.cm,
                  align: :center, valign: :center
  text_box 'sun', at: [gutter_cm + hmargin_cm + 17.cm, page_height / 2 - vmargin_cm - 4.5.cm],
                  width: 1.cm, height: 0.45.cm,
                  align: :center, valign: :center
...
# Even pages
  repeat(->(pg) { pg.even? && pg > 1 }) do
    ...
    stamp_at 'dots', [hmargin_cm - dot_size_cm, page_height / 2 + vmargin_cm]
    ...
    text_box 'mon', at: [hmargin_cm + 5.cm, page_height / 2 - vmargin_cm],
                    width: 1.cm, height: 0.45.cm,
                    align: :center, valign: :center
    text_box 'tue', at: [hmargin_cm + 11.cm, page_height / 2 - vmargin_cm],
                    width: 1.cm, height: 0.45.cm,
                    align: :center, valign: :center
    text_box 'wed', at: [hmargin_cm + 17.cm, page_height / 2 - vmargin_cm],
                    width: 1.cm, height: 0.45.cm,
                    align: :center, valign: :center
  end

# Odd pages
  repeat(->(pg) { pg.odd? && pg > 1 }) do
    stroke do
      horizontal_line 0, page_width, at: page_height / 2
    end

    stamp_at 'dots', [gutter_cm + hmargin_cm - dot_size_cm, page_height / 2 + vmargin_cm]
  ...
  while true
    ...
    year_calendar << wweek.days.map { |d| format('%2d', d.mday) }.join('  ') + "\n"
    calendar = WeekCalendar.new(wweek).pdf_text
    text_box calendar, at: [hmargin_cm + 12.75.cm, page_height / 2 + vmargin_cm + 4.04.cm],
                       width: 6.cm, height: 4.cm,
                       inline_format: true,
                       align: :right, valign: :bottom,
                       leading: 1.15.mm
